...
inline void tcp_get_ipv4_headers(struct rte_mbuf *m, struct tcp_ip_hdrs *hdrs);
inline void tcp_ipv4_pkt_addr_to_str(uint32_t addr, char *strbuf);
inline void tcp_print_tcpipv4_pkt_info(struct rte_mbuf *m, bool data);
...
inline void tcp_print_tcpipv4_pkt_info(struct rte_mbuf *m, bool data) {
    ...
    if (data) {
        for (uint16_t i = 0; i < hdrs.tcp_datalen; ++i) {
            if (isprint(hdrs.tcp_data[i]) || isspace(hdrs.tcp_data[i])) {
